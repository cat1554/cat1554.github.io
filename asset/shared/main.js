var global_debugMode = 0;
var global_buttonLock = false;

var global_pageContent;

var global_source = "tlw";

var global_xmlobj;

var global_lastsel = "";

var global_menu_x1;
var global_menu_x2;
var global_menu_y1;
var global_menu_y2;

var global_replace_a;
var global_replace_b;
var global_replace_c;
var global_replace_d;
var global_replace_e;
var global_replace_f;
var global_replace_h;
var global_replace_i;
var global_replace_j;
var global_replace_k;
var global_replace_l;

var global_notreplace_a;
var global_notreplace_b;
var global_notreplace_c;
var global_notreplace_d;
var global_notreplace_e;
var global_notreplace_f;

var global_sys_aud_id_1 = 0;
var global_sys_aud_id_2 = "0";

var global_loop1;
var global_loop2;
var global_loop3;
var global_loop4;
var global_loop5;
var global_loop6;
var global_loop7;
var global_loop8;

var global_menuOpen;
var global_mouseLeaver = new Event("mouseleave");

var global_errorCache = [];
var global_notifCache = [];

var global_sys_aud_src = {
	menu_opn: "data:audio/mp3;base64,",
	menu_sel: "data:audio/mp3;base64,",
	menu_pop: "data:audio/mp3;base64,",
	menu_cls: "data:audio/mp3;base64,",
	menu_err: "data:audio/mp3;base64,",
	menu_new: "data:audio/mp3;base64,"
};

function global_debugTest(){
	if (global_debugMode != -1) {
		window.console.debug("HTML modified: " + document.lastModified);
		window.console.debug("Main JS modified: " + document.getElementById("script_lastedited").contentDocument.lastModified);
		window.console.debug("Main CSS modified: " + document.getElementById("style_lastedited").contentDocument.lastModified);
		window.console.debug("Navigator modified: " + document.getElementById("menu_lastedited").contentDocument.lastModified);
	}
}

function global_summonAudio2(obj) {
	document.getElementById(obj).addEventListener("ended", function() {
		document.getElementById(obj).remove();
	});
	document.getElementById(obj).addEventListener("error", function() {
		window.console.warn("Failed to load file " + document.getElementById(obj).src + " for item: " + obj);
		document.getElementById(obj).remove();
	});
}

function global_summonAudio(obj) {
	var global_sys_audio = document.createElement("audio");
	global_sys_aud_id_2 = String(global_sys_aud_id_1);
	global_sys_audio.id = global_sys_aud_id_2;
	global_sys_aud_id_1++;
	if (global_sys_aud_id_1 > 511.5) {
		global_sys_aud_id_1 = 0;
	}
	document.head.appendChild(global_sys_audio);
	global_summonAudio2(global_sys_aud_id_2);
	global_sys_audio.src = global_sys_aud_src[obj];
	global_sys_audio.play();
}

function global_press(xml, ancestry){
	if (global_debugMode != -1) {
		window.console.debug(xml, ancestry);
	}
	if (ancestry != "error") {
		switch(xml.behaviour) {
			case "Link":
				window.location.assign("http://" + location.host + "/" + xml.link);
				break;
			case "ExtLink":
				if (global_buttonLock == true){
					if (global_menuOpen == 1) {
						global_summonAudio("menu_cls");
					}
					global_lastsel = "no";
					global_menuOpen = 0;
					document.getElementById("menubox_" + ancestry).style.zIndex = 16;

					document.getElementById("menubox_" + ancestry).style.display = "none";
//					document.getElementById("menubox_" + xml.id).style.zIndex = 16;
					global_buttonLock = false;

					document.getElementById("menubox_" + ancestry + "_" + xml.id).className = "menubutton";
				}
				window.open("http://" + xml.link, '_blank');
				global_summonAudio("menu_new");
//				global_editMessage("You will be taken to:\n" + xml.link + "\nContinue?", {txt: "No", func: "close"}, {txt: "Yes", func: xml.link}, null);
				break;
			case "Procrastinate":
				if (global_buttonLock == true){
					if (global_menuOpen == 1) {
						global_summonAudio("menu_cls");
					}
					global_lastsel = "no";
					global_menuOpen = 0;
					document.getElementById("menubox_" + ancestry).style.zIndex = 16;

					document.getElementById("menubox_" + ancestry).style.display = "none";
//					document.getElementById("menubox_" + xml.id).style.zIndex = 16;
					global_buttonLock = false;

					document.getElementById("menubox_" + ancestry + "_" + xml.id).className = "menubutton";
				}
				break;
			default:
		}
	} else {
		switch(xml){
			case "close":
				document.getElementById("pageglobal_errorbg").style.display = "none";
				break;
			default:
				window.location.assign("http://" + xml);
		}
	}
}

function global_closeMenu(menu, resetItem){
	global_buttonLock == false;
}

function global_createButton(xml, parent, order, menu){
	global_replace_d = document.createElement("button");
	global_replace_d.id = "menubox_" + parent + "_" + xml.id;
	if (xml.behaviour == "OffLink") {
		global_replace_d.className = "offmenubutton";
	} else {
		global_replace_d.className = "menubutton";
	}
	global_replace_d.innerHTML = xml.text;
	global_replace_d.style.top = (order * 20) + "px";
//	window.console.debug(parent);
	global_elementThing = document.getElementById("menubox_" + parent);
	global_elementThing.appendChild(global_replace_d);

	if (xml.behaviour != "OffLink") {
		document.getElementById("menubox_" + parent + "_" + xml.id).addEventListener("mouseover", function(a){
			if (global_buttonLock == false){
				document.getElementById("menubox_" + parent + "_" + xml.id).className = "menubuttonlight";
				if (a.target.id == "menubox_" + parent + "_" + xml.id) {
					if (global_lastsel != "menubox_" + parent + "_" + xml.id) {
						global_summonAudio("menu_pop");
						global_lastsel = "menubox_" + parent + "_" + xml.id;
					}
				}
			}
		});
	} else {
		document.getElementById("menubox_" + parent + "_" + xml.id).addEventListener("mouseover", function(a){
			if (global_buttonLock == false){
//				document.getElementById("menubox_" + parent + "_" + xml.id).className = "menubuttonlight";
				if (a.target.id == "menubox_" + parent + "_" + xml.id) {
					if (global_lastsel != "menubox_" + parent + "_" + xml.id) {
//						global_summonAudio("menu_pop");
						global_lastsel = "menubox_" + parent + "_" + xml.id;
					}
				}
			}
		});
	}

	document.getElementById("menubox_" + parent + "_" + xml.id).addEventListener("click", function(a){
		if (xml.behaviour != "Label") {
			if (xml.behaviour != "OffLink") {
				if (global_buttonLock == false){
					global_buttonLock = true;
					document.getElementById("menubox_" + parent + "_" + xml.id).className = "menubuttonselect";
					if (a.target.id == "menubox_" + parent + "_" + xml.id) {
						global_summonAudio("menu_sel");
					}
					document.getElementById("script_lastedited").contentWindow.setTimeout(global_press, 500, xml, parent);
				}
			} else {
				if (a.target.id == "menubox_" + parent + "_" + xml.id) {
	if (global_debugMode != -1) {
		window.console.debug(xml, parent, menu);
	}
						global_summonAudio("menu_err");
				}
			}
		}
	});

	if (xml.behaviour != "OffLink") {
		document.getElementById("menubox_" + parent + "_" + xml.id).addEventListener("mouseleave", function(){
			if (global_buttonLock == false){
				document.getElementById("menubox_" + parent + "_" + xml.id).className = "menubutton";
			}
		});
	}
}

function global_createSubMenuContent(xml, source, topmenu, order){
//	Main frame

//	window.console.debug("xml=", xml, "\nsrc=" + source + "\ntop=" + topmenu + "\nord=" + order);
/*
	for (let global_menusLoopSubMenu = 0; global_menusLoopSubMenu < xml.menuContents.length; global_menusLoopSubMenu++) {
		global_buildMenu(xml.menuContents[global_menusLoopSubMenu], ancestry + "_" + xml.id + "_container", global_menusLoopSubMenu, xml.id);
	} */
}

function global_createSubMenu(xml, parent, order, topmenu){
	global_replace_d = document.createElement("button");
	global_replace_d.id = "menubox_" + parent + "_" + xml.id;
	global_replace_d.className = "menubutton";
	global_replace_d.innerHTML = xml.text;
	global_replace_d.style.top = (order * 20) + "px";
//	window.console.debug(parent);
	global_elementThing = document.getElementById("menubox_" + parent);
	global_elementThing.appendChild(global_replace_d);

	global_replace_e = document.createElement("img");
	global_replace_e.id = "menubox_" + parent + "_" + xml.id + "_arr";
	global_replace_e.className = "menusvg";
	global_replace_e.src = "/asset/shared/arr.svg";
	global_replace_e.style.position = "absolute";
	global_replace_e.style.top = "6px";
	global_replace_e.style.right = "6px";
	global_replace_e.style.width = "8px";
	global_replace_e.style.height = "8px";
//	window.console.debug(parent);
	global_elementThing = global_replace_d;
	global_elementThing.appendChild(global_replace_e);

	global_replace_i = document.createElement("div");
	global_replace_i.id = "menubox_" + parent + "_" + xml.id + "_container";
	global_replace_i.className = "menucont";
	global_replace_i.style.position = "absolute";
	global_replace_i.style.top = "0px";
	global_replace_i.style.left = "100%";
	global_replace_i.style.width = xml.width + "px";
	global_replace_i.style.height = (xml.depth * 20) + "px";
	global_elementThing = document.getElementById("menubox_" + parent + "_" + xml.id);
	global_elementThing.appendChild(global_replace_i);

//	window.console.debug(topmenu);

	document.getElementById("menubox_" + parent + "_" + xml.id + "_container").style.display = "none";

	for (let global_menusLoopSubMenuTop = 0; global_menusLoopSubMenuTop < xml.menuContents.length; global_menusLoopSubMenuTop++) {
//		global_createSubMenuContent(xml.menuContents[menusLoopSubMenuTop], "menubox_" + parent + "_container_" + xml.id, topmenu, menusLoopSubMenuTop);

		global_buildMenu(xml.menuContents[global_menusLoopSubMenuTop], parent + "_" + xml.id + "_container", global_menusLoopSubMenuTop, topmenu);
	}


	document.getElementById("menubox_" + parent + "_" + xml.id).addEventListener("mouseover", function(a){
		if (global_buttonLock == false){
			document.getElementById("menubox_" + parent + "_" + xml.id).className = "menubuttonlight";
			document.getElementById("menubox_" + parent + "_" + xml.id + "_container").style.display = "initial";
			if (a.target.id == "menubox_" + parent + "_" + xml.id) {
			//	window.console.debug("menubox_" + parent + "_" + xml.id);
			//	window.console.debug(a.target.id);
				if (global_lastsel != "menubox_" + parent + "_" + xml.id) {
					global_summonAudio("menu_opn");
					global_summonAudio("menu_pop");
					global_lastsel = "menubox_" + parent + "_" + xml.id;
				}
			}
		}
	});

	document.getElementById("menubox_" + parent + "_" + xml.id).addEventListener("click", function(a){
		if (xml.behaviour != "Label") {
			if (a.target.id == "menubox_" + parent + "_" + xml.id) {
				if (global_buttonLock == false){
					global_buttonLock = true;
					document.getElementById("menubox_" + parent + "_" + xml.id).className = "menubuttonselect";
					global_summonAudio("menu_sel");

					document.getElementById("script_lastedited").contentWindow.setTimeout(global_press, 500, xml, parent);
				}
			}
		}
	});

	document.getElementById("menubox_" + parent + "_" + xml.id).addEventListener("mouseleave", function(){
		if (global_buttonLock == false){
			document.getElementById("menubox_" + parent + "_" + xml.id).className = "menubutton";
			document.getElementById("menubox_" + parent + "_" + xml.id + "_container").style.display = "none";
		}
	});
}

function global_editMessage(p,e1,e2,e3){
	document.getElementById("pageglobal_errortxt").innerHTML = p;
	if (e2 != null) {
		document.getElementById("pageglobal_errorbutton2").style.display = "initial";
		document.getElementById("pageglobal_errorbutton2").innerHTML = e2.txt;

		document.getElementById("pageglobal_errorbutton2").addEventListener("click", function(a){
			global_press(e2.func, "error");
		});
	} else {
		document.getElementById("pageglobal_errorbutton2").style.display = "none";
	}
	if (e3 != null) {
		document.getElementById("pageglobal_errorbutton3").style.display = "initial";
		document.getElementById("pageglobal_errorbutton3").innerHTML = e3.txt;

		document.getElementById("pageglobal_errorbutton3").addEventListener("click", function(a){
			global_press(e3.func, "error");
		});
	} else {
		document.getElementById("pageglobal_errorbutton3").style.display = "none";
	}
	if (e1 != null) {
		document.getElementById("pageglobal_errorbutton1txt").innerHTML = e1.txt;

		document.getElementById("pageglobal_errorbutton1").addEventListener("click", function(a){
			global_press(e1.func, "error");
		});
		document.getElementById("pageglobal_errorbg").style.display = "initial";
	
		global_summonAudio("menu_err");
	}
}

function global_createMessage(){
	global_replace_e = document.createElement("div");
	global_replace_e.id = "pageglobal_errorbg";
	global_elementThing = document.body;
	global_elementThing.appendChild(global_replace_e);

	document.getElementById("pageglobal_errorbg").style.display = "none";

	global_replace_f = document.createElement("div");
	global_replace_f.id = "pageglobal_errorwidth";
	global_elementThing = global_replace_e;
	global_elementThing.appendChild(global_replace_f);

	global_replace_e = document.createElement("div");
	global_replace_e.id = "pageglobal_errorbox";
	global_elementThing = global_replace_f;
	global_elementThing.appendChild(global_replace_e);

	global_replace_f = document.createElement("img");
	global_replace_f.id = "pageglobal_errorimg";
	global_replace_f.src = "asset/shared/error_icon.svg";
	global_elementThing = global_replace_e;
	global_elementThing.appendChild(global_replace_f);

	global_replace_f = document.createElement("p");
	global_replace_f.id = "pageglobal_errortxt";
	global_elementThing = global_replace_e;
	global_elementThing.appendChild(global_replace_f);

	global_replace_f = document.createElement("button");
	global_replace_f.id = "pageglobal_errorbutton1";
	global_elementThing = global_replace_e;
	global_elementThing.appendChild(global_replace_f);

	global_replace_h = document.createElement("p");
	global_replace_h.id = "pageglobal_errorbutton1txt";
	global_elementThing = global_replace_f;
	global_elementThing.appendChild(global_replace_h);

	global_replace_f = document.createElement("button");
	global_replace_f.id = "pageglobal_errorbutton2";
	global_elementThing = global_replace_e;
	global_elementThing.appendChild(global_replace_f);

	global_replace_f = document.createElement("button");
	global_replace_f.id = "pageglobal_errorbutton3";
	global_elementThing = global_replace_e;
	global_elementThing.appendChild(global_replace_f);

	document.getElementById("pageglobal_errorbg").addEventListener("click", function(a){
		if (a.target.id == "pageglobal_errorbg") {
			global_summonAudio("menu_err");
		}
	});
	document.getElementById("pageglobal_errorwidth").addEventListener("click", function(a){
		if (a.target.id == "pageglobal_errorwidth") {
			global_summonAudio("menu_err");
		}
	});
}

function global_buildMenu(xml, parent, order, menu) {
	// Top button
//	window.console.debug(xml);
	switch(xml.type) {
		case "button":
			global_createButton(xml, parent, order, menu);
			break;
		case "submenu":
			global_createSubMenu(xml, parent, order, menu);
			break;
	}
}

function global_initMenu(xml) {
//	Main frame
//	window.console.debug(xml);

	global_replace_a = document.createElement("div");
	global_replace_a.id = "menubox_" + xml.id;
	global_replace_a.className = "menubox";
	global_replace_a.style.position = "absolute";
	global_replace_a.style.top = xml.y + "px";
	global_replace_a.style.left = xml.x + "px";
	global_replace_a.style.width = xml.width + "px";
	global_replace_a.style.height = ((xml.depth * 20) + 20) + "px";

	global_elementThing = document.body;
	global_elementThing.appendChild(global_replace_a);

	global_replace_b = document.createElement("button");
	global_replace_b.id = "menubox_" + xml.id + "_opener";
	global_replace_b.className = "menuopenbutton";
	global_replace_b.innerHTML = xml.title;
	global_elementThing = document.getElementById("menubox_" + xml.id);
	global_elementThing.appendChild(global_replace_b);

	global_replace_c = document.createElement("div");
	global_replace_c.id = "menubox_" + xml.id + "_container";
	global_replace_c.className = "menucont";
	global_replace_c.style.position = "absolute";
	global_replace_c.style.top = "20px";
	global_replace_c.style.bottom = "0px";
	global_replace_c.style.left = "0px";
	global_replace_c.style.width = "100%";
	global_elementThing = document.getElementById("menubox_" + xml.id);
	global_elementThing.appendChild(global_replace_c);
	
	document.getElementById("menubox_" + xml.id + "_container").style.display = "none";

	document.getElementById("menubox_" + xml.id + "_opener").addEventListener("click", function(){
		if (global_buttonLock == false){
			global_menuOpen = 1;
			global_summonAudio("menu_opn");
			document.getElementById("menubox_" + xml.id).style.zIndex = 128;
			document.getElementById("menubox_" + xml.id + "_container").style.display = "initial";
		}
	});

	document.getElementById("menubox_" + xml.id).addEventListener("mouseleave", function(e){
		if (global_debugMode == 1) {
			window.console.debug(e);
		}
		if (global_buttonLock == false){
			if (global_menuOpen == 1) {
				global_summonAudio("menu_cls");
			}
			global_lastsel = "no";
			global_menuOpen = 0;
			document.getElementById("menubox_" + xml.id + "_container").style.display = "none";
			document.getElementById("menubox_" + xml.id).style.zIndex = 16;
		}
	});

	for (let global_menusLoop = 0; global_menusLoop < xml.menuContents.length; global_menusLoop++) {
		global_buildMenu(xml.menuContents[global_menusLoop], xml.id + "_container", global_menusLoop, xml.id);
	}
}

function global_parseMenu(xml) {
	for (let global_menusTopLoop = 0; global_menusTopLoop < xml.contents.length; global_menusTopLoop++) {
		global_initMenu(xml.contents[global_menusTopLoop]);
	}
}

function global_notifSys() {
	
}

function global_initNotif() {
	global_notreplace_a = document.createElement("div");
	global_notreplace_a.id = "pageglobal_headbar_notificationbox";
	global_elementThing = document.getElementById("pageglobal_headbar");
	global_elementThing.appendChild(global_notreplace_a);

/*	notreplace_b = document.createElement("div");
	notreplace_b.id = "pageglobal_headbar_notificationbox";
	elementThing = document.getElementById("pageglobal_headbar");
	elementThing.appendChild(notreplace_a);
*/

	document.getElementById("style_lastedited").contentWindow.setInterval(global_notifSys, 1000);
}

const global_xmlhttp = new XMLHttpRequest();
global_xmlhttp.onload = function() {
//	window.console.debug(this.responseText);
	const global_myObj = JSON.parse(this.responseText);
//	window.console.debug(global_myObj);
	global_xmlobj = global_myObj;
	global_parseMenu(global_myObj);
	global_initNotif();

//	xmlobj["contents"][0];
};
global_xmlhttp.onerror = function() {
	window.console.exception(" X X \n     \n XXX \nX   X\nFailed to load the menu contents.\nLoading cannot continue.\nTHIS IS NORMAL BEHAVIOUR IF THIS FILE IS LOADED LOCALLY.");
};

function initGlobal() {
	global_debugMode = ((window.location.search).indexOf("debug=1"));

	global_pageContent = document.body.innerHTML;
	document.body.innerHTML = "";

	global_replace_a = document.createElement("div");
	global_replace_a.id = "pageglobal_headbar";
	global_elementThing = document.body;
	global_elementThing.appendChild(global_replace_a);

/*	global_replace_a = document.createElement("base");
	global_replace_a.href = "https://www.cat1554.github.io/tlw_test/";
	global_replace_a.target = "_self";
	global_elementThing = document.body;
	global_elementThing.appendChild(global_replace_a);
*/

	global_replace_a = document.createElement("div");
	global_replace_a.id = "pageglobal_contentframe";
	global_replace_a.style.zIndex = 32;
	global_elementThing = document.body;
	global_elementThing.appendChild(global_replace_a);

//	replace_b = document.createElement("div");
//	replace_b.id = "pageglobal_contentbuffer1";
//	elementThing = replace_a;
//	elementThing.appendChild(replace_b);

//	replace_b = document.createElement("div");
//	replace_b.id = "pageglobal_contentbuffer2";
//	elementThing = replace_a;
//	elementThing.appendChild(replace_b);

	global_replace_b = document.createElement("div");
	global_replace_b.id = "pageglobal_content";
	global_elementThing = global_replace_a;
	global_elementThing.appendChild(global_replace_b);

	global_replace_b.innerHTML = global_pageContent;

	global_replace_c = document.createElement("iframe");
	global_replace_c.id = "script_lastedited";
	global_replace_c.style.display = "none";
	global_replace_c.src = "/asset/shared/main.js";
	global_elementThing = global_replace_a;
	global_elementThing.appendChild(global_replace_c);

	global_replace_c = document.createElement("iframe");
	global_replace_c.id = "style_lastedited";
	global_replace_c.style.display = "none";
	global_replace_c.src = "/asset/shared/main.css";
	global_elementThing = global_replace_a;
	global_elementThing.appendChild(global_replace_c);

	global_replace_c = document.createElement("iframe");
	global_replace_c.id = "menu_lastedited";
	global_replace_c.style.display = "none";
	global_replace_c.src = "/asset/shared/menuservice/menucontent.json";
	global_elementThing = global_replace_a;
	global_elementThing.appendChild(global_replace_c);

	global_createMessage();

	window.console.debug("debug: " + global_debugMode);

	document.title = "tlw" + window.location.pathname;

	global_xmlhttp.open("GET", "/asset/shared/menuservice/menucontent.json");
	global_xmlhttp.send();

	document.getElementById("script_lastedited").contentWindow.setTimeout(global_debugTest, 1000);
}
